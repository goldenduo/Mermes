...
| | +-------------------+ +-------------------+                   | |
| | |  🧠 长期记忆      | |  ⚙️ 设置          |                   | |
| | |  - Swipe Card     | |  - Preference List|                   | |
| | |  - BottomSheet    | |  - Gateway Config |                   | |
| | +-------------------+ +-------------------+                   | |
| |                                                               | |
| |                 [Bottom Navigation Bar]                       | |
| |          [Chat]   [Kanban]   [Memory]   [Settings]            | |
| +---------------------------------------------------------------+ |
+---------------------------------------------------------------+
```
...
* **原生交互与系统集成**：
  ...
  - **测试连接失败原因提示**：点击”测试连接”按钮后，如果连接失败，系统必须在 UI 上显示具体的失败原因，而非仅显示”连接失败”。失败原因包括但不限于：
    ...
    - 密钥解析失败（密钥格式错误、口令不匹配）
    - 主机密钥验证失败（首次连接或密钥变更）
  - **连接模式选择界面持久化**：用户从主界面”断开连接”或”退出登录”并返回到连接模式选择界面后，下次启动应用时必须仍然停留在连接模式选择界面，不得自动跳过选择直接进入主界面。即：只有在用户主动完成连接模式选择并成功连接后，才记录”已选择”状态；一旦用户返回选择界面，应清除该状态，确保下次启动重新进入选择流程。
...
### 3.7 设置页面 (Settings Page)
* **界面定位与原生布局**：
  - 全局系统设置与网关配置。底部导航栏的”设置” Tab，采用标准 Android Preference 风格的滚动设置列表，包含中英文切换、日志脱敏开关等全局设置项，以及**平台网关配置**区域。
* **原生交互与系统集成**：
  - **中英文 UI 热切换 (Bilingual Hot Swap)**：用户在设置中选择”中文 (简体)”或”English”后，应用免重启立即无缝刷新所有界面文本。
  - **发布级别日志脱敏开关 (Safe Decoupled Log Toggle)**：开启后，底层 `common` 模块自动激活脱敏规则，隐去敏感目录和数据结构实现，确保对外导出的应用崩溃日志或控制台日志绝对安全合规。
  - **平台网关配置 (Gateway Config)**：设置页面内含”平台网关”配置区域，采用折叠手风琴面板（Expandable Accordion List），每个三方通信平台独立成栏。
